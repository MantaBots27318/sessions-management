...
1.2 Ensure Non Registration Of Event With No Attendees
    ${scenario}      Load Scenario Data    9             test/data/conf_non_full.json
...
1.4 Ensure Registration Of Updated Event With Only Attendees Change
    ...
    ${result}        Run Registration Workflow with Mocks    ${scenario}    moi@moi.com    test@test.org
    ${scenario}      Update Scenario Data From Results   ${scenario}    ${result}
    ${scenario2}     Load Scenario Data    10            test/data/conf_non_full.json
...
1.5 Ensure No Registration Of Updated Event With Less Attendees
    ...
    ${result}        Run Registration Workflow with Mocks    ${scenario}    moi@moi.com    test@test.org
    ${scenario}      Update Scenario Data From Results   ${scenario}    ${result}
    ${scenario2}     Load Scenario Data    12            test/data/conf_non_full.json
    ${scenario}      Merge Scenario Data                 ${scenario}    ${scenario2}
    ${reference}     Load Results          Nothing       test/data/conf_non_full.json
    ${result}        Run Registration Workflow with Mocks    ${scenario}    moi@moi.com    test@test.org
    Check Final State        ${reference}    ${result}
